...
func (self *LockManager) PushLockAof(lock *Lock, aofFlag uint16) error {
	...
	err := self.lockDb.aofChannels[protocol.MurmurHash3KeySum(self.lockKey)%uint32(self.lockDb.managerMaxGlocks)].Push(lock.manager.dbId,
		lock, protocol.COMMAND_LOCK, lock.command, nil, aofFlag, lock.manager.AofLockData(protocol.COMMAND_LOCK, lock))
	if err != nil {
...
func (self *LockManager) PushUnLockAof(dbId uint8, lock *Lock, lockCommand *protocol.LockCommand, unLockCommand *protocol.LockCommand, isAof bool, aofFlag uint16) error {
	...
	err := self.lockDb.aofChannels[protocol.MurmurHash3KeySum(self.lockKey)%uint32(self.lockDb.managerMaxGlocks)].Push(dbId, lock,
		protocol.COMMAND_UNLOCK, lockCommand, unLockCommand, aofFlag, lock.manager.AofLockData(protocol.COMMAND_UNLOCK, lock))
	if err != nil {
		self.lockDb.slock.Log().Errorf("Database lock push aof error DbId:%d LockKey:%x LockId:%x",
			lock.command.DbId, lock.command.LockKey, lock.command.LockId)
		return err
