...
func (self *LockDB) resizeWaitRemoveLockManagerQueue() {
	for i := int64(0); i < WAIT_REMOVE_LOCK_MANAGER_QUEUE_LENGTH; i++ {
		self.waitRemoveLockManagers[i] = make([]*LockManagerQueue, self.managerMaxGlocks)
		for j := uint16(0); j < self.managerMaxGlocks; j++ {
			self.waitRemoveLockManagers[i][j] = NewLockManagerQueue(4, 16, WAIT_REMOVE_LOCK_MANAGER_INIT_SIZE)
		}
	}
}
...
func (self *LockDB) checkTimeTimeOut(checkTimeoutTime int64, now int64, glockIndex uint16, doTimeoutLockQueues []*LockQueue) {
	...
	for lock != nil {
		self.doTimeOut(lock, false, false)
		lock = doTimeoutLocks.Pop()
	}
...
func (self *LockDB) checkMillisecondTimeOut(ms int64, glockIndex uint16) {
	...
	for i := range lockQueue.IterNodes() {
		...
		for j, lock := range nodeQueues {
			if lock != nil {
				self.doTimeOut(lock, false, true)
				nodeQueues[j] = nil
			}
		}
	}
...
func (self *LockDB) checkTimeExpried(checkExpriedTime int64, now int64, glockIndex uint16, doExpriedLockQueues []*LockQueue) {
	...
	for lock != nil {
		self.doExpried(lock, false, false)
		lock = doExpriedLocks.Pop()
	}
...
func (self *LockDB) checkMillisecondExpried(ms int64, glockIndex uint16) {
	...
	for i := range lockQueue.IterNodes() {
		...
		for j, lock := range nodeQueues {
			if lock != nil {
				self.doExpried(lock, false, true)
				nodeQueues[j] = nil
			}
		}
	}
...
func (self *LockDB) flushWaitRemoveLockManagerQueue(glockIndex uint16) {
	for i := int64(0); i < WAIT_REMOVE_LOCK_MANAGER_QUEUE_LENGTH; i++ {
		...
		_ = self.waitRemoveLockManagers[i][glockIndex].Reset()
	}
}
...
func (self *LockDB) checkTimeWaitRemoveLockManager(timeIndex int, glockIndex uint16) {
	self.managerGlocks[glockIndex].HighPriorityLock()
	...
	_ = lockManagerQueue.Rellac()
	self.managerGlocks[glockIndex].HighPriorityUnlock()
}

func (self *LockDB) addWaitRemoveLockManager(lockManager *LockManager) {
	timeIndex := ((time.Now().UnixMilli() + 1000) % (WAIT_REMOVE_LOCK_MANAGER_QUEUE_LENGTH * 500)) / 500
...
func (self *LockDB) GetOrNewLockManager(command *protocol.LockCommand) *LockManager {
	fastHash := (uint32(command.LockKey[0]) | uint32(command.LockKey[1])<<8 | uint32(command.LockKey[2])<<16 | uint32(command.LockKey[3])<<24) ^ (uint32(command.LockKey[4]) | uint32(command.LockKey[5])<<8 | uint32(command.LockKey[6])<<16 | uint32(command.LockKey[7])<<24) ^ (uint32(command.LockKey[8]) | uint32(command.LockKey[9])<<8 | uint32(command.LockKey[10])<<16 | uint32(command.LockKey[11])<<24) ^ (uint32(command.LockKey[12])<<24 | uint32(command.LockKey[13])<<16 | uint32(command.LockKey[14])<<8 | uint32(command.LockKey[15]))
	fastValue := &self.fastLocks[fastHash%self.fastKeyCount]
	...
	if atomic.CompareAndSwapUint32(&fastValue.lock, 0, 1) {
		...
		atomic.StoreUint32(&fastValue.lock, 2)
...
func (self *LockDB) doExpried(lock *Lock, forcedExpried bool, removeWaited bool) {
	...
	if !forcedExpried {
		if self.status != STATE_LEADER && lock.isAof {
...
func (self *LockDB) UnLock(serverProtocol ServerProtocol, command *protocol.LockCommand, lockPriorityLevel uint8) error {
	...
	*/
	...
	if currentLock.locked > 1 {
		...
		if command.Rcount > 0 && command.TimeoutFlag&protocol.TIMEOUT_FLAG_RCOUNT_IS_PRIORITY == 0 {
			currentLock.locked--
			lockManager.locked--
