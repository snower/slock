...
func TestAdmin_Parse(t *testing.T) {
    admin := &Admin{}
    admin_parse := &AdminParser{make([]byte, 4096), 0, 0, 0, make([]string, 0), 0, make([]byte, 0), 0}
...
func TestAdmin_MultiParse(t *testing.T) {
   admin := &Admin{}
   admin_parse := &AdminParser{make([]byte, 4096), 0, 0, 0, make([]string, 0), 0, make([]byte, 0), 0}
...
func TestAdmin_BuildResult(t *testing.T) {
    admin := &Admin{}
    r := admin.BuildResult(true, "OK", nil)
    if string(r) != "+OK\r\n" {
        t.Errorf("Admin Build Success Result Fail %s", string(r))
        return
    }

    r = admin.BuildResult(false, "Unknown Command", nil)
    if string(r) != "-ERR Unknown Command\r\n" {
        t.Errorf("Admin Build Error Result Fail %s", string(r))
        return
    }

    r = admin.BuildResult(true, "", []string{"Success"})
    if string(r) != "$7\r\nSuccess\r\n" {
        t.Errorf("Admin Build Result Fail %s", string(r))
        return
    }

    r = admin.BuildResult(true, "", []string{"Success", "test"})
    if string(r) != "*2\r\n$7\r\nSuccess\r\n$4\r\ntest\r\n" {
