...
func run2(client net.Conn, count *int, wcount *int, max_count int, end_count *int, clock *sync.Mutex, index uint32) {
    ...
    for i :=0; i<64; i++ {
        ...
        wbuf[i*64+37], wbuf[i*64+38], wbuf[i*64+39], wbuf[i*64+40], wbuf[i*64+41], wbuf[i*64+42], wbuf[i*64+43], wbuf[i*64+44] = byte(0), byte(0), byte(0), byte(bench_count), byte(index>>24),  byte(index>>16), byte(index>>8), byte(index)
