...
func run2(client net.Conn, count *uint32, wcount *uint32, maxCount uint32, waiter chan bool, index uint32) {
    ...
    for i :=0; i<64; i++ {
        ...
        wbuf[i*64+37], wbuf[i*64+38], wbuf[i*64+39], wbuf[i*64+40], wbuf[i*64+41], wbuf[i*64+42], wbuf[i*64+43], wbuf[i*64+44] = byte(0), byte(0), byte(0), byte(benchCount), byte(index>>24),  byte(index>>16), byte(index>>8), byte(index)
